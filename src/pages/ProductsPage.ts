...
import {PRODUCT_PAGE_LOCATORS} from '@/locators/Products'
...
export class ProductsPage extends BasePage {
  ...
  async verifyPageTitle(expectedTitle: string = 'Products'): Promise<void> {
    await test.step('Verify page title', async () => {
      await this.validateText(PRODUCT_PAGE_LOCATORS.title, expectedTitle)
    })
  }

  async addItemToCart(item: string): Promise<void> {
    await test.step('Add item to cart', async () => {
      await this.clickOnElement(PRODUCT_PAGE_LOCATORS.addToCartItem(item))
    })
  }

  async proceedToCartPage(): Promise<void> {
    await test.step('Proceed to cart page', async () => {
      await this.clickOnElement(PRODUCT_PAGE_LOCATORS.cartButton)
  ...
  async logout(): Promise<void> {
    await test.step('Logout', async () => {
      await this.clickOnElement(PRODUCT_PAGE_LOCATORS.openHamburgerMenu)
      await this.clickOnElement(PRODUCT_PAGE_LOCATORS.logoutButton)
